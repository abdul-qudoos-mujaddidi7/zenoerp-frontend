<script>
    ...
    async function loadProductionDetails() {
        ...
        loading = false;
        console.log("Production details loaded:", {
            production,
            production_items,
            production_payments,
            production_returns,
            production_return_items,
            production_journals,
            production_stock_transactions,
        });
    }
    async function doDeleteProduction() {
        saving = true;
        try {
            // Mark production as deleted
            let deletedProduction = await db.productions
                .where("id")
                .equals(production.id)
                .first();
            await db.productions.update(production.id, { status: 0 });

            // Mark associated production items as deleted
            let itemsToDelete = await db.production_items
                .where("production_id")
                .equals(production.id)
                .and((item) => item.status === 1)
                .toArray();
            for (const item of itemsToDelete) {
                await db.production_items.update(item.id, { status: 0 });
            }

            // Mark associated payments as deleted

            let paymentsToDelete = await db.production_payments
                .where("production_id")
                .equals(production.id)
                .and((p) => p.status === 1)
                .toArray();
            for (const p of paymentsToDelete) {
                await db.production_payments.update(p.id, { status: 0 });
            }

            // Mark associated returns as deleted

            let returnsToDelete = await db.production_returns
                .where("production_id")
                .equals(production.id)
                .and((r) => r.status === 1)
                .toArray();
            for (const r of returnsToDelete) {
                await db.production_returns.update(r.id, { status: 0 });
            }
            ...
            if (returnsToDelete.length > 0) {
                ...
                let returnItemsToDeleteFromReturn = await db.production_return_items
                    .where("production_return_id")
                    .anyOf(returnIds)
                    .and((ri) => ri.status === 1)
                    .toArray();
            ...
            for (const ri of returnItemsToDelete) {
                await db.production_return_items.update(ri.id, { status: 0 });
            }
            ...
            // Mark production_return journals as deleted
            for (const ret of production_returns) {
            ...
            // Mark production_payment journals as deleted
            for (const p of paymentsToDelete) {
            ...
            // Mark production_return stock transactions as deleted
            for (const ret of production_returns) {
            ...
            await logActivity({
                ...
                description: `Deleted production #${production.id}`,
            });

            Swal.fire({
                icon: "success",
                title: t("Deleted!"),
                text: t("Production deleted successfully."),
            });
            ...
            Swal.fire({
                icon: "error",
                title: t("Error"),
                text: t("An error occurred while deleting the production."),
                confirmButtonText: t("OK"),
            });
            // alert(t("An error occurred while deleting the production."));
        } finally {
            saving = false;
        }
    }
