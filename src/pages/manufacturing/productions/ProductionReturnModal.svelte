<script>
  ...
  export let production = null; // parent passes production object
  ...
  onMount(async () => {
    ...
    warehouse = await db.warehouses.where({ id: production.warehouse_id, status: 1 }).first();
    account = await db.accounts.where({ id: production.account_id, status: 1 }).first();
    currencies = await db.currencies.where({ status: 1 }).toArray();
    currency = production.currency || currencies[0]?.code || 'AFN';
    items = await db.production_items
      .where('production_id')
    ...
    returns = await db.production_returns.where({ production_id: production.id, status: 1 }).toArray();
    let tempReturnsWithItems = [];
    for (const r of returns) {
      // Ensure type match for production_return_id
      const items = await db.production_return_items.where({ production_return_id: Number(r.id), status: 1 }).toArray();
  ...
  async function saveReturn() {
    if (saving) return;
    if (!production || !warehouse || !account) {
      Swal.fire({
        icon: 'error',
        title: t('Error'),
        text: t('Missing production, warehouse, or account.'),
    ...
    try {
      ...
      await db.transaction(
        'rw',
        [
          db.production_returns,
          db.production_return_items,
        ...
        async () => {
          ...
          const returnId = await db.production_returns.add({
          ...
          await logActivity({
            ...
            description: `Created production return #SR-${returnId} for production #${production.production_number}`,
          });

          // Add return items and stock transactions
          for (const i of validItems) {
            await db.production_return_items.add({
              production_return_id: returnId,
            ...
            await db.stock_transactions.add({
              ...
              expiry_date: null,
              heaviness: null,
              description: `Production return for Production #${production.production_number}`,
              status: 1,
              updated_at: new Date().toISOString(),
            });
            addedItems.push(i);
          ...
          const productionsAccount = await db.accounts
            .where('code')
            .equals('SALES')
            .and((s) => s.status === 1)
            .first();

          if (!productionsAccount) {
          ...
          await db.journals.add({
            ...
            reference_id: returnId,
            reference_type: 'production_return',
            description: `Production Return Transaction for Production #${production.production_number}`,
            currency: currency,
          ...
          if (account.id == WalkinAccount.id) {
            ...
            //     reference_id: returnId,
            //     reference_type: "production_return",
            //     description: `Payment for Productions Return #${returnId} of Production #${production.production_number}`,
            //     currency: currency,
            //     first_entry_account: productionsAccount.id,
      ...
      Swal.fire({
        icon: 'success',
        title: t('Success'),
        text: t('Production return recorded successfully.'),
        confirmButtonText: t('OK'),
      });
      dispatch('close');
    } catch (err) {
      Swal.fire({
        icon: 'error',
        title: t('Error'),
        text: t('Error saving production return: ') + err.message,
        confirmButtonText: t('OK'),
      });
    } finally {
      saving = false;
