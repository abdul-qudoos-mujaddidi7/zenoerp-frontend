      ...
      if (result) {
        db.documents.update(doc.id, { status: 0, file: '', thumbnail: '' }).then(async () => {
          ...
          await logActivity({
            ...
            new_values: null,
            description: 'Deleted appointment document entry for appointment ID ' + id,
          });
        });
      }
    });
    ...
    if (!editingId) {
      ...
      db.documents.put(newDoc).then(async () => {
        ...
        await logActivity({
          ...
          description: 'Created new appointment document entry for appointment ID ' + id,
        });
        loadDocuments();
...
<div class="modal fade" id="viewImageModal" bind:this={viewModalRef} tabindex="-1" aria-hidden="true">
          ...
          <div class="col-md-12 mb-4">
            {#if viewImageUrl}
              <div class="text-center mb-3">
                <img src={viewImageUrl} alt="Preview" class="" style="max-height: 500px;" />
              </div>
            {/if}
          </div>
        </div>
