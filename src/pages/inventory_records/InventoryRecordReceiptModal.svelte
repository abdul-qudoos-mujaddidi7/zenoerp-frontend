<script>
  ...
  async function loadItems() {
    // load inventory_record items and attach product names
...
* {color:#000000 !important;}
  ...
  $: total = Number(
    inventory_record?.total_amount || (items && items.length ? items.reduce((s, i) => s + (Number(i.subtotal) || 0), 0) : 0),
  );
  ...
  <div class="modal-dialog modal-dialog-centered" style="max-width: 310px;">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">{t('Receipt')} — {inventory_record?.invoice_number}</h5>
        ...
        <div id="receipt-content">
          <div class="receipt">
            ...
            <div class="mb-2">
              {t('Date')}:
              <span class="fw-bold"
                >{@html inventory_record?.invoice_date ? showDate(inventory_record?.invoice_date.slice(0, 10), 'receipt') : ''}</span>
              <small class="float-end"> <span class="fw-bold">{inventory_record?.invoice_date.slice(0, 10)}</span></small>
            </div>

            <div class="float-end">
              <small>{t('Invoice #')}: <span class="fw-bold">{inventory_record?.invoice_number}</span></small>
            </div>
            ...
            <div>
              <small
                >{t('Customer')}:
                <span class="fw-bold">{getAccountName(customer) || inventory_record?.description || 'Walk-in'}</span></small>
            </div>
            ...
            <div class="totals small mt-3">
              ...
              <div style="display:flex;justify-content:space-between;margin-bottom:6px;">
                <div class="fw-bold">{t('Discount')} {inventory_record?.discount_type=='percent' ? `(${Number(inventory_record?.discount_amount).toLocaleString(undefined, { maximumFractionDigits: 3 })}%)` : ''}</div>
                <div class="right fw-bold">
                 
                  {inventory_record?.discount_amount
                    ? calculateDiscountAmount(inventory_record).toLocaleString(undefined, { maximumFractionDigits: 3 })
                    : 0}
              ...
              <div style="display:flex;justify-content:space-between;margin-bottom:6px;">
                ...
                <div class="right fw-bold">
                  {inventory_record?.expense_amount
                    ? Number(inventory_record?.expense_amount).toLocaleString(undefined, { maximumFractionDigits: 3 })
                    : 0}
                  {t(inventory_record?.currency) || ''}
                </div>
              </div>
              ...
              <div style="display:flex;justify-content:space-between;margin-bottom:6px;">
                ...
                <div class="right fw-bold">
                  {inventory_record?.total_amount
                    ? Number(inventory_record?.total_amount).toLocaleString(undefined, { maximumFractionDigits: 3 })
                    : 0}
                  {t(inventory_record?.currency) || ''}
                </div>
              </div>
              ...
              <div style="display:flex;justify-content:space-between;margin-top:4px;">
                ...
                <div class="right fw-bold">
                  <span dir="ltr"
                    >{balance[inventory_record?.currency]
                      ? Number(balance[inventory_record?.currency] + due || 0).toLocaleString(undefined, {
                          maximumFractionDigits: 3,
                        })
                      : '---'}</span>
                  {t(inventory_record?.currency) || ''}
      ...
      <div class="modal-footer">
        ...
        <button
          ...
          on:click={() => {
            ...
            WhatsappModalRef.openModal('inventory_record_created_template', inventory_recordData, customer, balance);
